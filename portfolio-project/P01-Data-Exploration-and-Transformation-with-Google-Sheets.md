...
1. `ID` ใช้
   ...
   ผมขออธิบายส่วน `"\d-\d{4}-\d{5}-\d{2}-\d"` โดยใช้ข้อมูลลูกค้าแถวที่ 5 เป็นผลลัพธ์ตัวอย่าง
...
2. `Gender` ใช้
   ...
   มขออธิบายส่วน `"M[a-z]+."` โดยใช้ข้อมูลลูกค้าแถวที่ 5 เป็นผลลัพธ์ตัวอย่าง
...
4. `DOB` ใช้
    ...
    ซึ่งผมขอเอาข้อมูลลูกค้าแถวที่ 5 ออกมาเป็นผลลัพธ์
    ซึ่งหมายถึง..

    ดึงตัวเลข 0-9 มา 2 ตัว (ผลลัพธ์ `14`)
    
    - ตามด้วย `.` ที่แทนอะไรก็ได้ 1 ตัว (ผลลัพธ์ `14 `) และ
    - ตามด้วย `[A-z]+` ตัวอักษรภาษาอังกฤษตัวเล็กหรือตัวใหญ่กี่ตัวก็ได้ (ผลลัพธ์ `14 Feb`)
    - ตามด้วย `.` ที่จะแทนถึง white space แต่จริงมันแทนด้วยอะไรก็ได้เช่นเดียวกัน (ผลลัพธ์ `14 Feb `)
    - ตามด้วย `\d{4}` แทนด้วยตัวเลข 0-9 4 ตัวเลข (ผลลัพธ์ `14 Feb 1992`)
...
7. `Address` ใช้
    ...
    จากสูตรข้างต้น ภายในการจัดกลุ่มคือ `\d+ [A-z]+ [A-z]+..+ ` ผมขอใช้กับข้อมูลลูกค้าแถวที่ 5 เพื่อดึงที่อยู่ออกมาเป็นผลลัพธ์

    `\d+ ` หา  pattern และดึง ค่าตัวเลข 0-9 มากี่ตัวก็ได้ (ผลลัพธ์ `7878`)
    ตามด้วย white space 1 เคาะ space bar (ผลลัพธ์ `7878 `)
    ตามด้วย `[A-z]+ ` เอาตัวอักษรภาษาอังกฤษตัวใหญ่หรือตัวเล็ก เอามากี่ตัวก็ได้ 
    ตามด้วย white space 1 เคาะ
    ตามด้วย `[A-z]+` เอาตัวอักษรภาษาอังกฤษตัวใหญ่หรือตัวเล็ก เอามากี่ตัวก็ได้
