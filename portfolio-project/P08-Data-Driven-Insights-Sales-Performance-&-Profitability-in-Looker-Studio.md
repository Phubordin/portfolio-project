...
🔥🔥**ขั้นตอนที่ 1** : อัพเดท `Price`, `Cost`, `Profit`, `%NPM`, `%ROI` ต่อหน่วย ให้มีแค่ One-to-One กับ Product ID เช่นกัน
...
โดยใช้สูตร Excel ต่อไปนี้เพื่ออัพเดทคอลัมน์ : `Sales`

```{excel}
=ArrayFormula((Quantity * Price per Unit) * (1-Discount))
...
```{excel}
=ArrayFormula(Sales - (Cost per Unit * Quantity))
...
1. จัดการความสัมพันธ์จาก Many-to-Many ระหว่างคอลัมน์ Product ID และ Product Name ให้เป็น One-to-One พร้อมอัพเดทชื่อสินค้าใหม่กับตารางหลัก
2. อัพเดทคอลัมน์ `Price`, `Cost`, `Profit`, `%NPM`, `%ROI` ใหม่เพื่อป้องกัน 1 `Product ID` มีได้หลายราคา
3. ต่อเนื่องจากข้อ 2 ถ้าหากอัพเดทราคาต่อหน่วยแล้ว ต้องอัพเดทคอลัมน์ `Profit` และ `Sales` ด้วย เพราะมีบาง Transaction ที่มีราคาขายของสินค้าไม่เท่ากันทำให้ Profit, Sales
   อาจจะ diff กันได้

📌 ปล. ที่เราต้องมาจัดการข้อมูลแบบนี้เพราะ ข้อมูลนี้มันไม่ใช่ข้อมูลจริงตามที่ผมเกรื่นนำไว้เบื้องต้น มันเป็นข้อมูลที่ใช้ฝึกฝนจาก Tableau ดังนั้น มันอาจจะทำให้เกิดการจับคู่ที่ผิดเพี้ยนไปบ้าง
...
แต่ความเป็นจริง เราต้องไป Check อีกทีว่าสินค้าเรามี ID และ Name ตรงกันไหม ราคาเป็นยังไง ต้นทุนเป็นยังไง ที่จะส่งผลให้ %NPM และ %ROI นั้นถูกต้อง

- 1 SKU จะมีหลายเป็น Product Name ไม่ได
- 1 Product Name จะมีหลาย SKU ไม่ได้
- ทั้ง SKU และ Product Name ต้องมี ราคาขาย และ ต้นทุนขาย ที่เท่ากัน (หากแต่เกิดเหตุการณ์ที่ต้องเปลี่ยนจริงๆก็ต้องสอบถามคนคิดราคาขายและต้นทุนใหม่)

โปรเจกต์นี้เราจะเชื่อเลยว่า ทุกๆ Product ID, Product Name, Price, Cost ต้องมีความสัมพันธ์แบบ One-to-One
